...
const UpsellRelatedProductCard: React.FC<productcartProps> = ({ data }) => {
  ...
  const hanleClick = () => {
    router.push(`/product/${data.id}/${data.linkedProduct.name}`);
  };
  // product preview
  ...
  const onAddToCart: MouseEventHandler<HTMLButtonElement> = (event) => {
    ...
    cart.addItem({
      ...
      size: selectedValue,
      color: selectedColor,
      price: +data.linkedProduct.price,
      name: data.linkedProduct.name,
      image: data.linkedProduct.Images[0].url,
    });
  };
    ...
    <>
      <div className="bg-white group cursor-pointer  p-3 space-y-4 text-center">
        <div className="aspect-square rounded-xl bg-gray-100 relative">
          <Image
            src={data.linkedProduct.Images[0].url}
            alt="E-commerce"
            fill
        ...
        <div>
          <div>
            <p className="font-semibold text-lg">{data.linkedProduct.name}</p>
            <p className="text-sm text-gray-500">
              {data.linkedProduct.category?.name}
            </p>
          </div>
          {/* Price & Reiew */}
...
export default UpsellRelatedProductCard;
